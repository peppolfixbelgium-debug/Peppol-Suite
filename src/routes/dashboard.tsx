...
function Dashboard() {
  ...
  return <main className="mx-auto max-w-5xl px-4 py-10">
    <div><h1 className="font-display text-3xl tracking-tight">Account</h1><p className="mt-2 text-muted">Your profile, subscription, usage and conversion history.</p></div>
    <section className="mt-8 grid gap-4 lg:grid-cols-[1.1fr_.9fr]">
      <div className="rounded-2xl border border-border bg-elevated p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="font-display text-2xl">Profile</h2><p className="mt-1 text-sm text-muted">Account details from your authenticated session.</p></div>{user.role === "admin" ? <span className="rounded-full bg-accent-soft px-3 py-1 text-xs font-semibold text-accent">Admin test mode</span> : null}</div><dl className="mt-5 grid gap-4 sm:grid-cols-2"><div><dt className="text-xs uppercase tracking-wide text-muted">Name</dt><dd className="mt-1 break-words">{user.name || "Not provided"}</dd></div><div><dt className="text-xs uppercase tracking-wide text-muted">Email</dt><dd className="mt-1 break-words">{user.email}</dd></div><div><dt className="text-xs uppercase tracking-wide text-muted">Email verification</dt><dd className="mt-1">{user.emailVerified ? "Verified" : "Not verified"}</dd></div><div><dt className="text-xs uppercase tracking-wide text-muted">Account role</dt><dd className="mt-1 capitalize">{user.role}</dd></div></dl></div>
      <div className="rounded-2xl border border-border bg-elevated p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="font-display text-2xl">Plan</h2><p className="mt-1 text-sm text-muted">Current subscription and monthly allowance.</p></div><span className="rounded-full bg-accent-soft px-3 py-1 text-xs font-semibold text-accent">{displayPlan}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-1"><div className="rounded-xl border border-border bg-surface p-4"><p className="text-xs uppercase tracking-wide text-muted">Subscription</p><p className="mt-1 font-medium">{displayPlan} plan</p></div><div className="rounded-xl border border-border bg-surface p-4"><p className="text-xs uppercase tracking-wide text-muted">Billing</p><p className="mt-1 font-medium">Monthly allowance</p><p className="mt-1 text-xs text-muted">Live payments are not enabled.</p></div></div></div>
    </section>
    <section className="mt-6 rounded-2xl border border-border bg-elevated p-5"><div><h2 className="font-display text-2xl">Usage</h2><p className="mt-1 text-sm text-muted">Usage resets with the monthly quota period. Bulk usage is measured in document units.</p></div>{usage ? <div className="mt-5 grid gap-4 md:grid-cols-2"><UsageCard label="Conversions" used={usage.conversions.used} limit={usage.conversions.limit} /><UsageCard label="Bulk documents" used={usage.bulk.used} limit={usage.bulk.limit} /></div> : <p className="mt-5 text-sm text-muted">Usage is temporarily unavailable.</p>}</section>
