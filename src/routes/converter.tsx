...
import { canConsume, consumeQuota, getQuota } from "@/lib/peppol/quota";
...
export const Route = createFileRoute("/converter")({
  validateSearch: (s: Record<string, unknown>): Search => {
    ...
    const sample =
      ...
      raw === "1" ||
      raw === 1 ||
      (typeof raw === "string" && raw.replaceAll('"', "") === "1");
...
function ConverterPage() {
  ...
  async function onFile(file: File) {
    ...
    setData(extractInvoice(extracted.fullText));
    setTab("fields");
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <section className={cn("rounded-2xl border border-border bg-elevated p-4", tab !== "pdf" && "hidden lg:block")}>
          ...
          {progress ? (
            <p className="mt-3 flex items-center gap-2 text-sm text-muted">
              ...
              {t(lang, "extracting", { page: progress.page, total: progress.total })}
            </p>
          ) : null}
        ...
        <section
          className={cn(
            "max-h-[80vh] overflow-auto rounded-2xl border border-border bg-surface p-4",
            tab !== "fields" && "hidden lg:block",
          )}
        >
          <h2 className="mb-4 text-sm font-medium">{t(lang, "fields")}</h2>
          <InvoiceForm data={data} onChange={setData} lang={lang} />
          <div className="mt-6">
            <h3 className="mb-2 text-sm font-medium">{t(lang, "issues")}</h3>
        ...
        <section className={cn("flex flex-col rounded-2xl border border-border bg-elevated p-4", tab !== "xml" && "hidden lg:block")}>
          ...
          <pre className="mt-3 max-h-[70vh] flex-1 overflow-auto rounded-xl bg-bg p-3 font-mono text-[11px] leading-relaxed text-fg">
            {xml}
          </pre>
        </section>
      </div>
      ...
      {showUpgrade ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-fg/40 p-4">
          <div className="w-full max-w-md rounded-2xl border border-border bg-elevated p-6">
            <h3 className="font-display text-2xl">{t(lang, "quota_full")}</h3>
            <p className="mt-2 text-sm text-muted">{t(lang, "convert_sub")}</p>
            <div className="mt-5 flex flex-wrap gap-2">
              <Button type="button" variant="outline" onClick={() => setShowUpgrade(false)}>
                {t(lang, "continue")}
              </Button>
              <Button asChild>
