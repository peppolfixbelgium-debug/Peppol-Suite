...
function AuthSlot() {
  ...
  if (user) return <div className="flex items-center gap-3"><Link to="/dashboard" className="hidden text-sm text-muted hover:text-fg sm:inline">Account</Link>{user.role === "admin" ? <Link to="/command-center" className="hidden text-sm font-medium text-accent hover:underline sm:inline">Command Center</Link> : null}<Link to="/dashboard" className="hidden max-w-32 truncate text-sm text-muted hover:text-fg sm:inline">{user.name || user.email}</Link><button className="text-sm text-muted hover:text-fg" onClick={() => useAuth.getState().signOut()}>{t(lang, "nav_logout")}</button></div>;
...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="flex min-h-dvh flex-col bg-bg text-fg">
    ...
    </div>{open&&<div className="border-t border-border px-4 py-3 md:hidden"><div className="flex flex-col gap-1">{NAV.map(item=><Link key={item.to} to={item.to} onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm">{t(lang,item.key)}</Link>)}{user ? <Link to="/dashboard" onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm">Account</Link> : null}{user?.role === "admin" ? <Link to="/command-center" onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm font-medium text-accent">Command Center</Link> : null}{!user ? <Link to="/login" onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm">{t(lang,"nav_login")}</Link> : null}</div></div>}</header>
