...
export function Shell({ children }: { children: ReactNode }) {
  ...
  return <div className="flex min-h-dvh flex-col bg-bg text-fg">
    ...
    </div>{open&&<div className="border-t border-border px-4 py-3 md:hidden"><div className="flex flex-col gap-1">{NAV.map(item=><Link key={item.to} to={item.to} onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm">{t(lang,item.key)}</Link>)}<Link to="/dashboard" onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm">{t(lang,"nav_dashboard")}</Link>{user?.role === "admin" ? <Link to="/command-center" onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm font-medium text-accent">Command Center</Link> : null}<Link to="/login" onClick={()=>setOpen(false)} className="rounded-md px-2 py-3 text-sm">{t(lang,"nav_login")}</Link></div></div>}</header>
