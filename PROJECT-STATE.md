...
- Main now includes the latest engineering/pricing/legal/company preparation plus authenticated Account/profile navigation and document-based bulk quota enforcement.
...
- Admin/Founder test mode now uses the same usage counters with a 1,000,000-unit test ceiling so QA usage reflects actual tested documents instead of always reporting zero.
- Account usage UI labels bulk allowance explicitly as **Bulk documents** to match the backend semantics.
- CI verification must be treated separately from documentation/evidence commits; no CI GREEN claim is made for the latest checkpoint until a workflow is independently observed.
...
- GitHub workflow lookup for the latest main checkpoint returned no workflow runs, so CI is not claimed GREEN here.
