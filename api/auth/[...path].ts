...
import { getDb, getDefaultFreePlanId, requireEnv } from "../_lib/db.js";
...
export async function verifyIdToken(provider: "google" | "microsoft", token: string, clientId: string, nonce: string): Promise<JwtClaims> {
  ...
  } else if (!/^https:\/\/login\.microsoftonline\.com\/[^/]+\/v2\.0$/.test(claims.iss) || !claims.oid || !claims.tid) throw new Error("Invalid Microsoft issuer or identity claims.");
...
async function signup(request: Request) {
  ...
  const rows = await sql<{ id: string }[]>`INSERT INTO users (email, name, password_hash, plan_id) VALUES (${email}, ${name || null}, ${passwordHash}, ${planId}) RETURNING id`;
...
async function oauthCallback(request: Request, provider: "google" | "microsoft") {
  ...
  if (decision === "create") {
    ...
    const planId = await getDefaultFreePlanId(sql);
    const created = await sql<any[]>`INSERT INTO users (email, name, plan_id, email_verified_at) VALUES (${email}, ${claims.name ?? null}, ${planId}, ${provider === "google" ? new Date().toISOString() : null}) RETURNING id, email, name, role, plan_id, email_verified_at, disabled_at`;
    user = created[0];
...
async function handler(request: Request) {
  ...
  if (path === "oauth/google/link/start" && request.method === "POST") {
    const user = await getSessionUser(request);
    if (!user) return json({ error: "Authentication required." }, 401);
    return oauthStart(request, "google", user.id);
  }
  if (path === "oauth/microsoft/link/start" && request.method === "POST") {
    const user = await getSessionUser(request);
    if (!user) return json({ error: "Authentication required." }, 401);
    return oauthStart(request, "microsoft", user.id);
  }
...
async function authBoundary(request: Request): Promise<Response> {
  try {
    return await handler(request);
  } catch (error) {
    if (isThrownResponse(error)) {
...
export async function GET(request: Request): Promise<Response> { return authBoundary(request); }
export async function POST(request: Request): Promise<Response> { return authBoundary(request); }
