...
export async function handleUsageRequest(request: Request): Promise<Response> {
  const user=await getSessionUser(request);if(!user)return json({error:"Authentication required."},401);const sql=getDb();
  if(request.method==="GET"){
    ...
    return json({conversions:{used:Number(row?.conversions_used??0),limit:admin?ADMIN_TEST_LIMIT:Number(row?.monthly_conversion_limit??0)},bulk:{used:Number(row?.bulk_used??0),limit:admin?ADMIN_TEST_LIMIT:Number(row?.monthly_bulk_limit??0)},adminTestMode:admin});
  }
  if(request.method!=="POST")return json({error:"Method not allowed."},405);requireSameOrigin(request);const input=await request.json().catch(()=>null) as {kind?:unknown}|null;if(input?.kind!=="bulk"&&input?.kind!=="conversion")return json({error:"Invalid usage kind."},400);
  if(input.kind==="bulk"){
    const [plan]=await sql<any[]>`SELECT u.plan_id,p.monthly_bulk_limit AS limit FROM users u JOIN plans p ON p.id=u.plan_id WHERE u.id=${user.id}`;
    if(user.role==="admin") return json({kind:"bulk",used:Number(plan?.limit??0),limit:ADMIN_TEST_LIMIT,adminTestMode:true});
    if(plan?.plan_id==="free")return json({error:"Bulk conversion is not available on the Free plan."},403);const limit=Number(plan?.limit??0);const rows=await sql<any[]>`INSERT INTO usage_quota (user_id,period_start,conversions_used,bulk_used) VALUES (${user.id},date_trunc('month',current_date)::date,0,1) ON CONFLICT (user_id,period_start) DO UPDATE SET bulk_used=usage_quota.bulk_used+1 WHERE usage_quota.bulk_used<${limit} RETURNING bulk_used`;if(!rows[0])return json({error:"Monthly bulk limit reached."},429);return json({kind:"bulk",used:Number(rows[0].bulk_used),limit});
  }
