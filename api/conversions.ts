...
async function handleConversionRequest(request: Request): Promise<Response> {
  try {
    ...
    if (input.kind !== "conversion" && input.kind !== "bulk") return json({ error: "Invalid conversion kind." }, 400);
    ...
    const [row] = await sql<any[]>`
      WITH quota AS (
        INSERT INTO usage_quota (user_id, period_start, conversions_used, bulk_used)
        VALUES (${user.id}, date_trunc('month', current_date)::date, 1, 0)
        ON CONFLICT (user_id, period_start) DO UPDATE
          SET conversions_used = usage_quota.conversions_used + 1
          WHERE usage_quota.conversions_used < ${limit}
