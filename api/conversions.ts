  ...
  try {
    ...
    if (request.method === "GET") {
      ...
      const [quota] = await sql<any[]>`
        SELECT q.conversions_used, p.monthly_conversion_limit
        ...
        WHERE u.id = ${user.id}
      `;
      return json({ conversions: rows, quota: { used: Number(quota?.conversions_used ?? 0), limit: Number(quota?.monthly_conversion_limit ?? 0) } });
    }
    ...
    await securityEvent(request, "conversion_created", user.id, { conversionId: row.id });
    return json({ conversion: row, quota: { used: Number(quotaRows[0].conversions_used), limit } }, 201);
  } catch (error) {
